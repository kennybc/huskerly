...
def get_all_users_from_userpool_with_org_id(org_id, user_pool_id=pool_id):
    ...
    members = [user for user in filtered_users if get_user_role(user, "MEMBER")]
    other_users = [
        user
...
def get_user_attributes(user_response):
    res = {attr["Name"]: attr["Value"] for attr in user_response["UserAttributes"]}
    return res
    ...
    if current_role == "ASSIST_ADMIN":
        ...
        if current_org_admin:
            client.admin_update_user_attributes(
                ...
                UserAttributes=[{"Name": "custom:OrgRoll", "Value": "ASSIST_ADMIN"}],
            )
        return True
    else:
        raise Exception(
            f"""Error while promoting user {
...
def demote_to_member(org_id: int, user_email: str) -> bool:
    ...
    if current_role == "SYS_ADMIN":
        ...
        client.admin_update_user_attributes(
            UserPoolId=pool_id,
            Username=user_email,
            UserAttributes=[{"Name": "custom:OrgRoll", "Value": "MEMBER"}],
        )
        return True
    else:
        raise Exception(
            f"""Error while demoting user {
...
def update_org_request(
    org_name: str, creator_email: str, current_user_email: str, status: str
) -> bool:
    with get_cursor() as cursor:
...
def join_org(org_id: int, user_email: str, role: str = "MEMBER") -> bool:
    with get_cursor() as cursor:
        ...
        if invite is None:
            raise Exception(
                f"""No invitation found for user {
        ...
        response = client.admin_update_user_attributes(
            ...
            UserAttributes=[
                ...
                {"Name": "custom:OrgId", "Value": str(org_id)},
                {"Name": "custom:OrgRoll", "Value": role},  # TODO: NEEDS TO BE FIXED
            ],
        )
...
def invite_org(
    org_id: int, invitee_email: str, inviter_email: str, lifetime: int = 86400
) -> bool:
    with get_cursor() as cursor:
        ...
        if get_user_permission_level(inviter_email, org_id) not in [
            "SYS_ADMIN",
            "ORG_ADMIN",
            "ASSIST_ADMIN",
        ]:
